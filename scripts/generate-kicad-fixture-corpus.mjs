...
const corpusRootRelative = path
  .relative(repoRoot, corpusRoot)
  .replaceAll(path.sep, "/");
...
function fixturePath(fixture, ...segments) {
  return path.join(corpusRoot, fixture.id, ...segments);
}
...
function fixtureRelativePath(...segments) {
  return [corpusRootRelative, ...segments].join("/");
}
...
function filesForFixture(fixture) {
  ...
  const expectedDir = fixturePath(fixture, "expected");
  files.set(
...
function corpusManifest() {
  return {
    schemaVersion: 1,
    generatedBy: generatorPath,
    linearIssue: "OASLANA-36",
    githubIssue: 37,
    root: corpusRootRelative,
...
function writeCorpus(files) {
  ...
  console.log(`Wrote ${files.size} files to ${corpusRootRelative}`);
}

function checkCorpus(files) {
  const expectedPaths = new Set([...files.keys()]);
  const actualPaths = new Set(listActualFiles(corpusRoot));
  const errors = [];
